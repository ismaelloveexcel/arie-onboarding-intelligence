...
def test_explicit_imported_management_company_is_high_confidence():
    matches = match_introducers(
        _lead(management_company="Example Corporate Services Ltd"),
        [_introducer(company_name="Example Corporate Services Limited")],
    )
    assert len(matches) == 1
    assert matches[0]["match_type"] == "exact_name"
    assert matches[0]["match_strength"] == "high"
    assert "explicitly imported company route" in matches[0]["evidence"]
